...
class StateRandomVariable(RandomVariable):
    ...
    def sample_reparameterized(self, bs, num_particles):
        state = st.State()
        for name, rand_var in self.named_rand_var():
            setattr(state, name, rand_var.sample_reparameterized(bs=bs, num_particles=num_particles))
        return state
...
class MultivariateIndependentLaplace(RandomVariable):
    ...
    def logpdf(self, value, bs, num_particles):
        x = -torch.abs(value - self._location)
        x = x / self._scale - torch.log(2 * self._scale)
        x = x.view(bs, num_particles, -1)
...
class MultivariateIndependentNormal(RandomVariable):
    ...
    def logpdf(self, value, batch_size, num_particles):
        x = -0.5 * (value - self._mean)**2 / self._variance - 0.5 * torch.log(2 * self._variance * np.pi)
...
class Laplace(RandomVariable):
    ...
    def sample_reparameterized(self, batch_size, num_particles):
        return self._multivariate_independent_laplace.sample_reparameterized(bs, num_particles)
